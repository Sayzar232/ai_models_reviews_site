...
const LocalModelCard = {
    render(model) {
        ...
        if (!desc && model.author) desc = `Модель от ${model.author}`;
                ...
                <div class="model-card-header">
                    ...
                    <div class="model-card-title-group">
                        ...
                        <div style="font-size: 11px; color: var(--text-muted); margin-top: 4px;">
                            <i data-lucide="cpu" style="width:12px;height:12px;display:inline-block;vertical-align:middle;margin-right:2px"></i>
                            ${model.parameters_approx > 0 ? model.parameters_approx + 'B' : '?'} 
                            <span style="margin: 0 4px">•</span>
                            <i data-lucide="download" style="width:12px;height:12px;display:inline-block;vertical-align:middle;margin-right:2px"></i>
                            ${(model.downloads || 0).toLocaleString('ru-RU')}
                        </div>
                    </div>
                </div>
    ...
    initCharts(models) {
        models.forEach(model => {
            ...
            const labels = ['Код', 'Скор.', 'VRAM', 'Конт.', 'Креат.', 'Точн.'];
