/* ============================================
   LocalReviewList — paginated review feed for local models
   ============================================ */
...
const LocalReviewList = {
    ...
    async render(modelId, page = 1) {
        ...
        try {
            const data = await API.getLocalReviews(modelId, page, 10);
            if (!data.reviews || data.reviews.length === 0) {
                return `<div class="empty-state"><i data-lucide="message-square-off"></i><h3>Пока нет отзывов</h3><p>Станьте первым, кто оставит отзыв об этой модели!</p></div>`;
            }
    ...
    renderItem(review, idx) {
        ...
        const badges = [
            {l:'Код',v:review.score_coding},
            {l:'Скор.',v:review.score_speed},
            {l:'Логика',v:review.score_reasoning},
            {l:'Конт.',v:review.score_context},
            {l:'Инстр.',v:review.score_instruction},
            {l:'Точн.',v:review.score_accuracy}
        ...
        return `<div class="review-item" style="animation-delay:${idx*0.06}s">
            ...
            <div class="review-votes" style="display:flex;align-items:center;gap:12px;margin-top:16px;padding-top:16px;border-top:1px solid var(--border-subtle)">
                <button class="vote-btn ${review.user_vote === 1 ? 'active' : ''}" onclick="LocalReviewList.handleVote(${review.id}, 1)">
    ...
    async handleVote(reviewId, vote) {
        ...
        try {
            const btnContainer = document.querySelector(`.review-item button[onclick="LocalReviewList.handleVote(${reviewId}, ${vote})"]`)?.parentElement;
            if (btnContainer) {
                 btnContainer.style.opacity = '0.5';
            }
            
            await API.voteLocalReview(reviewId, vote);
    ...
    renderPagination(data) {
        ...
        h += `<button class="page-btn" ${data.page<=1?'disabled':''} onclick="LocalReviewList.goToPage(${data.page-1})"><i data-lucide="chevron-left"></i></button>`;
        for (let i=1;i<=data.pages;i++) {
            h += `<button class="page-btn ${i===data.page?'active':''}" onclick="LocalReviewList.goToPage(${i})">${i}</button>`;
        }
        h += `<button class="page-btn" ${data.page>=data.pages?'disabled':''} onclick="LocalReviewList.goToPage(${data.page+1})"><i data-lucide="chevron-right"></i></button>`;
        return h + '</div>';
    },

    async goToPage(page) {
        const c = document.getElementById('local-reviews-container');
        if (!c) return;
